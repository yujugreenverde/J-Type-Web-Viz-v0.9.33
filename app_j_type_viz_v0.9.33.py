...
# 6) 新增模板功能：可將調整後的介面參數儲存為 Template（JSON），並可載入套用。
#
# 其餘功能皆沿用 v0.9.33。
...
APP_VERSION = "0.9.36"
st.set_page_config(page_title="J-type-like Web Viz v0.9.36", layout="wide")
...
# -----------------------------------------------------------------------------
# Templates (save/apply UI state)
# -----------------------------------------------------------------------------
TEMPLATE_VERSION = 1
TEMPLATE_FILE_EXT = "jviz_template.json"

# Minimal template keys (style-only). Anything not listed here will be ignored in Minimal mode.
_TEMPLATE_KEYS_MINIMAL = {
    # Plot type & style/layout
    "plot_type", "fig_w", "fig_h", "title", "show_grid", "fontsize", "y_decimals",
    "enable_stats", "dec_p", "show_stars", "show_p_text",
    "enable_pub_style", "style_preset", "png_dpi",
    "caption_style", "stat_indicator", "auto_caption",

    # Common axis labels
    "xlabel", "ylabel",
}

# Keys that should never be saved/applied
_TEMPLATE_EXCLUDE_KEYS = {
    # data / heavy objects / uploaders
    "uploaded", "df", "df_plot", "fig", "ax", "summary_table",
    # template UI keys
    "tpl_uploader", "tpl_name", "tpl_mode", "tpl_scope",
}
...
def build_template_from_session_state(*, mode: str = "Full", scope: str = "Full plot") -> Dict[str, Any]:
    """Build a template from current st.session_state.

    mode:
      - "Full": save most JSON-able keys (except excluded).
      - "Minimal": save only style/layout-ish keys (exclude x/y/group choices).

    scope:
      - "Global style only": do not include x_col/y_col/group_col choices (even in Full mode)
      - "Full plot": include them (only in Full mode)
    """
    ss_raw = dict(st.session_state)

    include_columns = (mode == "Full") and (scope == "Full plot")

    out_ss: Dict[str, Any] = {}

    for k, v in ss_raw.items():
        if k in _TEMPLATE_EXCLUDE_KEYS:
            continue

        # In Minimal mode, only keep minimal allowlist keys
        if mode == "Minimal" and k not in _TEMPLATE_KEYS_MINIMAL:
            continue

        # In Global style only scope, drop column selection keys
        if not include_columns and k in {"x_col", "y_col", "group_col"}:
            continue

        # Avoid saving huge dynamically generated rename widgets unless user wants Full plot:
        # (rename_orig_*, rename_new_*)
        if mode == "Minimal" and (str(k).startswith("rename_orig_") or str(k).startswith("rename_new_")):
            continue
    ...
    return {
        ...
        "mode": mode,
        "scope": scope,
        "session_state": out_ss,
    }

def apply_template_to_session_state(tpl: Dict[str, Any]):
    """Apply template values into st.session_state.

    IMPORTANT: Call this BEFORE creating widgets, then st.rerun().
    """
    ss = tpl.get("session_state", {})
    ...
    for k, v in ss.items():
        if k in _TEMPLATE_EXCLUDE_KEYS:
            continue
        # Only apply JSON-able values (safety)
        if _is_jsonable(v):
...
# --- Sidebar ---
with st.sidebar:
    st.markdown("### 🧭 J-type Web Viz v0.9.36")

    # --- Templates (save/apply UI state) ---
    st.markdown("### 🧩 Templates")
    tpl_up = st.file_uploader(
        "Load template (.jviz_template.json)",
        type=["json"],
        key="tpl_uploader",
        help="Load a previously saved template to restore plot settings.",
    )

    c_t1, c_t2 = st.columns(2)
    with c_t1:
        tpl_mode = st.selectbox("Mode", ["Full", "Minimal"], index=0, key="tpl_mode",
                                help="Full: save most settings. Minimal: style/layout only.")
    with c_t2:
        tpl_scope = st.selectbox("Scope", ["Full plot", "Global style only"], index=0, key="tpl_scope",
                                 help="Global style only won't overwrite x/y/group column selections.")

    c_a, c_b = st.columns(2)
    with c_a:
        if st.button("Apply template", use_container_width=True, disabled=(tpl_up is None)):
            try:
                tpl = json.loads(tpl_up.getvalue().decode("utf-8"))
                apply_template_to_session_state(tpl)
                st.success("Template applied. Refreshing…")
                st.rerun()
            except Exception as e:
                st.error(f"Template apply failed: {e}")

    with c_b:
        tpl_name = st.text_input("Template name", value="my_template", key="tpl_name")
        if st.button("Build template", use_container_width=True):
            try:
                tpl = build_template_from_session_state(
                    mode=st.session_state.get("tpl_mode", "Full"),
                    scope=st.session_state.get("tpl_scope", "Full plot"),
                )
                safe_name = sanitize_filename(tpl_name)
                st.download_button(
                    "Download template",
                    data=json.dumps(tpl, ensure_ascii=False, indent=2).encode("utf-8"),
                    file_name=f"{safe_name}.{TEMPLATE_FILE_EXT}",
                    mime="application/json",
                    use_container_width=True,
                )
            except Exception as e:
                st.error(f"Template build failed: {e}")
...
with colC:
   ...
   # ---- 3️⃣ Custom editable caption ----
    ...
    st.markdown("---")
    st.caption("v0.9.36 • Added templates (save/apply UI state) + parameter report export in ALL.zip.")
# ---------------- rcParams for vector text -----------------
plt.rcParams.update({
    "pdf.fonttype": 42,
    "ps.fonttype": 42,
